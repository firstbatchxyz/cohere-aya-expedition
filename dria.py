...
class Dria:
    def __init__(
        self, num_nodes, base_instruction, base_model="microsoft/Phi-4-mini-instruct"
    ):
    ...
    def bootstrap(self):
        ...
        for node in self.nodes:
            node.set_seed()
            augmentation = node.generate(
                augmentation_prompt + self.base_instruction, self.max_steps
            )
    ...
    def guided_generation(self, use_previous_generation=False):
        augmentation_prompt = get_prompt("aug_v2")
        ...
        for node in tqdm(self.nodes, desc="Guided generation"):
            ...
            if use_previous_generation:
                instruction = node.last_generated

            augmentation = node.generate_with_guidance(
                augmentation_prompt + instruction,
            ...
            node.last_generated = augmentation
            self.data.append(augmentation)  # store generated data
            self.controller.add_concept(augmentation)
