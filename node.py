...
class LLMNode:
    def __init__(
        self, model, tokenizer, seed: int, node_id: str, end_tokens: List[str] = None
    ):
    ...
    def _tokenize_prompt(self, prompt: str):
        ...
        return self.tokenizer([txt], return_tensors="pt").input_ids.to(
            self.model.device
        )
    ...
    def apply_instruction(self, prompt: str):
        ...
        self._prompt_len = (
            self.sequence_ids.shape[-1] if self.sequence_ids is not None else 0
        )
    ...
    def sample_next_token_and_append(
        self, logits: torch.Tensor, temperature=1.0, top_p=1.0
    ):
        logits = (
            logits.to(self.model.device)
            if temperature <= 0
            else (logits / temperature).to(self.model.device)
        )
        if top_p < 1.0:
    ...
    def get_sequence_length(self):
        return 0 if self.sequence_ids is None else self.sequence_ids.shape[-1]
    ...
    def generate_with_guidance(
        ...
        for _ in range(max_steps):
            ...
            generated_suffix_ids = self.sequence_ids[:, self.get_prompt_length() :].to(
                self.model.device
            )

            if (
                negative_prompts
            ):  # Only compute negative logits if there are negative prompts
                ...
                for n_prompt_ids in neg_ids_list:
                    ...
                    logits_negative_cond = self.get_logits_for_sequence(
                        input_ids_negative_pass
                    )  # Logits on CPU
...
- Make sure the new prompt is different enough
    ...
    label = node.generate(
        classify_prompt.format(BASE=pos, AUGMENTATION=gen), 150, 1.0, 0.9
    )
    print(gen, label)
